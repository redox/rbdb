...
@x_offset  = true
  ...
  def self.swf_object(width, height, url)
    ...
    # i changed this line as well
    output += '<embed src="/open-flash-chart.swf?' + 'data=' + url.to_s + '" quality="high" bgcolor="#FFFFFF" width="' + width.to_s + '" height="' + height.to_s + '" id="open-flash-chart" name="open-flash-chart" align="middle" allowScriptAccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />'
    output += "\n"
    output += '</object>'
    output += "\n"
		return output
